...
class ChatService extends GetxService {
  ...
  Future<dynamic> getChatList(Map<String, dynamic> request) async {
    try {
      final response = await ApiManager().call(APIIndex.chatList, request, ApiType.post);
      if (response.status != 200 || response.data == null) {
        toaster.warning(response.message ?? 'Failed to load chats');
        return null;
      }
      return response.data;
    } catch (err) {
